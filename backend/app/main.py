...
from .models import (
    LoadModelRequest, LoadModelResponse, PredictionResponse,
    ExplainTextRequest, ExplainTextResponse, loaded_model_state
)
...
async def api_load_model(request: LoadModelRequest):
    ...
    try:
        ...
        loaded_model_state.update({
            "mode": request.mode, "model": model, "processor": processor,
            "model_id": actual_model_id, "device": actual_device,
        ...
        return LoadModelResponse(
            ...
            mode=request.mode, loaded_model_id=actual_model_id,
...
async def api_predict(
    text_input: Optional[str] = Body(None),
    audio_file: Optional[UploadFile] = File(None),
    state: Dict[str, Any] = Depends(get_model_state)
):
    preprocess_start_time = time.perf_counter()
    audio_data_tensor: Optional[torch.Tensor] = None
    ...
    try:
        ...
        if state["mode"] == "lfm2":
            generated_text = run_lfm2_prediction(
                text=text_input, audio_tensor=audio_data_tensor,
                sample_rate=sample_rate, model=state["model"], processor=state["processor"]
            )
        else: # text_shap mode
             ...
             generated_text = run_text_shap_prediction(
                 text=text_input, model=state["model"],
                 tokenizer=state["processor"], model_device=state["device"]
             )
...
async def api_explain_text(
    ...
    if state["mode"] != "text_shap":
        raise HTTPException(status_code=400, detail=f"SHAP is only available in 'text_shap' mode.")

    start_time = time.perf_counter()
    try:
        tokens, shap_values_list = explain_text(
            text_input=request.text_input, model_state=state, max_evals=request.max_evals
        )
